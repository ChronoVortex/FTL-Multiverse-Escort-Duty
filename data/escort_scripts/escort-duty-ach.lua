...
script.on_internal_event(Defines.InternalEvents.DRONE_COLLISION, function(drone, projectile, damage, response)
    if response.collision_type == 1 and drone.iShipId == 1 and drone.blueprint.typeName == "COMBAT" and should_track_achievement("ACH_SHIP_ESCORT_DUTY_2", Hyperspace.ships.player, "PLAYER_SHIP_ESCORT_DUTY") and userdata_table(projectile, "mods.escort.achTracking").firedFromProjDrone then
...
script.on_internal_event(Defines.InternalEvents.SHIP_LOOP, function(ship)
    if ship.iShipId == 0 and Hyperspace.playerVariables.loc_escort_damage_taken <= 5 and should_track_achievement("ACH_SHIP_ESCORT_DUTY_3", ship, "PLAYER_SHIP_ESCORT_DUTY") then
        ...
        if Hyperspace.playerVariables.loc_sector_count > 3 then
